  ...
  const faq = [
    {
      ...
      a: `אין תשובה אחת. ${formatNumber(atFloor.length)} שמות בישראל נושאים ${formatNumber(floor)} תושבים בלבד כל אחד, וכולם נדירים באותה מידה - אף אחד מהם אינו נדיר יותר מהאחרים. שמות שניתנו לפחות מ-5 אנשים בסך הכול לא נכללים בקובץ הלמ״ס כלל, כך שהשמות הנדירים באמת אינם מופיעים בשום רשימה ציבורית.`,
    },
    {
      q: 'כמה השם שלי נדיר?',
    ...
    {
      q: 'למה חלק מהשנים בעמוד של שם נדיר מסומנות כ-1-4 ולא במספר?',
      a: 'הלמ״ס מסתירה ספירות שנתיות של 1 עד 4 מטעמי פרטיות. באתר הן מוצגות כטווח 1-4 או כחסם תחתון, ולעולם לא כאפס ולא כמספר מדויק - דווקא בשמות נדירים זה קורה כמעט בכל שנה.',
    },
  ];
  ...
  return (
    <div className="py-4">
      ...
      <p className="pb-3 text-muted-foreground">
        השמות ברשימה ניתנו לעד 30 תושבי ישראל בסך הכול - אבל כולם היו בשימוש ב-15 השנים
